...
export const handleQnaUpload = async (req, res) => {
  try {
    ...
    const qnaPromises = questions.map(async (question, index) => {
      ...
      // Add audio URL to the review for frontend access
      if (audioUrl && reviews[index]) {
        reviews[index].audio = audioUrl;
      }
    ...
    // Create the review document with audio URLs included
    const review = await Review.create({
      ...
      strength: reviews.map(r => r.feedback.split('.')[0]),
      improvement: reviews.map(r => r.idealAnswer.split('.')[0]),
      qna: qnaIds,
      companyBased: {
      ...
      // Add the QnA details with audio URLs
      questionDetails: questions.map((question, index) => ({
        question,
        answer: transcripts[index] || "[No response]",
        audioUrl: uploadResults[index] ? uploadResults[index].secure_url : null
      }))
    });
    ...
    // Return the analysis to the client with audioUrls included
    return res.json({ 
      success: true,
      reviewId: review._id,
      reviews, // This now includes audio URLs added above
      questionDetails: review.questionDetails, // Include the questionDetails with audioUrls
